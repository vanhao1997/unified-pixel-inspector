...
class GTMClient {
     ...
     */
    async listFolders(workspacePath) {
        const result = await this.apiCall(`/${workspacePath}/folders`);
     ...
     */
    async createFolder(workspacePath, name) {
        return await this.apiCall(`/${workspacePath}/folders`, 'POST', { name });
    }

    /**
     * Get or create a folder by name
     * Returns the folder object with folderId
     */
    async getOrCreateFolder(workspacePath, folderName) {
        const existing = await this.listFolders(workspacePath);
        const found = existing.find(f => f.name === folderName);
        if (found) return found;
        return await this.createFolder(workspacePath, folderName);
    }
    ...
    async deploy(config) {
        ...
        try {
            ...
            // Step 2: Pre-create channel folders
            const folderCache = {}; // channelName -> folder object

            // Step 3: Create tags, each linked to the trigger and its channel folder
            for (const tagPayload of tagPayloads) {
                try {
                    ...
                    // Determine channel folder from tag name
                    const channelFolder = this.getChannelFolder(tagPayload.name);
                    if (channelFolder) {
                        try {
                            if (!folderCache[channelFolder]) {
                                folderCache[channelFolder] = await this.getOrCreateFolder(workspacePath, channelFolder);
                            }
                            tagPayload.parentFolderId = folderCache[channelFolder].folderId;
                        } catch (folderErr) {
                            // Non-critical: if folder creation fails, tag still works without folder
                            console.warn(`Could not create folder "${channelFolder}":`, folderErr.message);
                        }
                    }
    ...
    /**
     * Determine the channel folder name based on tag name prefix
     */
    getChannelFolder(tagName) {
        if (tagName.startsWith('Meta')) return 'Meta Pixel';
        if (tagName.startsWith('TikTok')) return 'TikTok Pixel';
        if (tagName.startsWith('GA4')) return 'Google Analytics 4';
        if (tagName.startsWith('Google Ads')) return 'Google Ads';
        if (tagName.startsWith('Zalo')) return 'Zalo Pixel';
        return null;
    }
}
